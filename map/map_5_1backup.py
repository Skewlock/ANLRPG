...
def create_map():
    ...
    for i in range(len(carte)):
        ...
        for j in range(len(carte[i])):
            if carte[i][j] != 0:
                L.append(pyglet.sprite.Sprite(dic_blocs[carte[i][j]], x=-32+ k + l, y=window.height/2-64 -k/2 + l/2 - 1,
                                              batch=batch, group=z))
                L[-1].update(scale = 2)
            k = k + 32
...
def get_coords_grid(mouse_):
    ...
    alpha = -math.atan(1/2)
    beta = math.atan(1/2)
    y2 = (mouse_[0]*math.sin(beta)-mouse_[1]*math.cos(beta))/(math.cos(alpha)*math.sin(beta)-math.sin(alpha)*math.cos(beta))
    print("-----")
    print(y2)
    x2 = (mouse_[1]*math.cos(alpha)-mouse_[0]*math.sin(alpha))/(math.cos(alpha)*math.sin(beta)-math.sin(alpha)*math.cos(beta))
    print(x2)
    ...
    for i in range(len(carte_decors)):
        ...
        for j in range (len(carte_decors[i])):
            if carte_decors[i][j] != 0:
                L_decors.append(pyglet.sprite.Sprite(dic_blocs[carte_decors[i][j]], x= -32 + k + l, y=window.height/2-64 - k/2 + l/2 - 1,
                                                     batch = batch, group = z))
